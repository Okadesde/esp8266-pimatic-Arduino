...
long sendInterval   = 30000; //in millis

String Username     = "ESPap";
...
void handle_root() {
  ...
  //server.send ( 200, "text/html", header + navbar + containerStart + data + containerEnd + siteEnd );
   server.send ( 200, "text/html", header + navbar + containerStart + title1 + Temperature + title2 + IPAddClient + DeviceType + ClientName + Interval + Uptime + title3 + IPAddServ + User + containerEnd + siteEnd);
}
